...
$a2->foo();// Код выведет 2. Потому что используется статическая переменная, после завершения работы метода она не удаляется, а сохраняет свое значение в самом классе, и так как это экземпляры одного класса то при каждом вызове метода любого экземпяра одного класса значение переменной $x всегда будет на единицу выше предыдушего вызова.
...
$b4->foo(); // Код выведет 1. Так как класс С потомок класса B, и вызывается метода уже объекта C то значение 1.  
...
/*Код выведет отже самое что и в пункте 6 При создании нового экземпляра класса если не передаются параметры скобки не обязательны конце 
